export default {
  ...
  state: {
    shader: `
      ...
      void main() {
        // Get Time
        highp float time = stream/20000. + sin(stream/2000.)/30.;
        ...
        // Zoom
        uv.xy *= sin(time*2.5 + dst*10.)/2. + .55;
        
        // Zoom out center
        //uv.xy *= 1./smoothstep(0., .2, 1.-dst);
        
        // Fisheye
        float strength = (abs(uv.x) * sin(stream/1000.) + 0.);
        uv = mix(uv, uv*2., dst/strength);
        ...
        // Color It In
        col = 0.5 + 0.5*sin(time*2.5 + vec3(0,2,4) + dst2*3.);
