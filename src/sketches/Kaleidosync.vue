...
export default {
  ...
  methods: {
    setSizeParams () {
      ...
      if (landscape) {
        this.state.maxSize = width / 2
      } else {
        this.state.maxSize = height / 2
      }
    ...
    setStarRadius (type) {
      let size = this.state.activeSize * this.getVolumeQueue('kaleidosync-beat')
    ...
    updateStar ({
      ...
      const update = (key, { val, interval }) => {
        ...
        star[key].get = () => interpolate(star[key].last, star[key].next)(ease(this.activeIntervals[interval + 's'].progress, this.state.easing, true))
      }
    ...
    setBackgroundColor (type) {
      ...
      this.state.background.get = () => {
        return interpolate(this.state.background.last, this.state.background.next)(this.activeIntervals[type + 's'].progress)
      }
    },
    ...
    paint (now) {
      ...
      const { width, height } = this.$refs.canvas
      this.$refs.canvas.ctx.fillStyle = `rgb(${r},${g},${b})`
      this.$refs.canvas.ctx.fillRect(0, 0, width, height)
      const beat = interpolateBasis([1, 1.2, 1])(ease(this.activeIntervals.beats.progress, 'easeOutCubic'))
      for (let star of this.state.stars) {
        ...
        this.$refs.canvas.ctx.fillStyle = `rgb(${r},${g},${b})`
        drawShape(this.$refs.canvas.ctx, vertices).fill()
      }
    }
  }
}
